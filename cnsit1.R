...
#############################################################################################
######### CN and system identity threat models

###### SI threat is more a function of narcissistic positivity toward the system/nation (CN)
###### than general positivity (patriotism / ingroup satisfaction)
### basic models
cs1<-lm_robust(rsit2 ~ rpatt2+rcnt2, data=Data, se_type = "HC3")
summary(cs1)
cs3<-lm_robust(rsit2 ~ rpatt2+rcnt2+age+gend+whitet1+rinct1+educ+rknt1+ridt2+rpid3t2, data=Data, se_type = "HC3")
summary(cs3)

### std coefficients, dominance analysis and Wald F for CN / IS comparison
### CN has stronger effect
summary(lm_robust(scale(rsit2) ~ scale(rpatt2)+scale(rcnt2)+age+gend+whitet1+rinct1+educ+rknt1+ridt2+rpid3t2, 
                  data=Data, se_type = "HC3"))
dominanceAnalysis(lm(rsit2 ~ rpatt2+rcnt2+age+gend+whitet1+rinct1+educ+rknt1+ridt2+rpid3t2, data=Data))
lht(cs3, test="F", "rpatt2 = rcnt2")

### figure
cns1 <- dwplot(cs3, dot_args = list(size = 1.5, shape=21, stroke = 1, color = "red", fill="white"),
  ...
  scale_x_continuous(breaks=seq(-0.2,0.6,0.2), limits = c(-0.2, 0.6)) + 
  labs(title="Predictors of Social Identity Threat", 
       caption="Source: 2020 National Survey") +
  theme(plot.title = element_text(size=14, hjust=0.5),
        legend.position = "none", aspect.ratio=1,
        panel.border = element_rect(color="black", fill=NA),
        strip.text = element_text(hjust=0.5)) 
ggsave(file="cnsfig1.png", cns1, width = 8, height = 10, type="cairo")

###### main SEM:
### baseline model and indirect effects
### 1. CN has signif indirect effect on conspiracy via SI threat
### 2. indirect effect of CN via SI threat > indirect effect of IS via SI threat
### 3. Total effect of CN on consp. larger in abs terms than total effect of IS
### 4. Total effects of CN and IS on conspiracy oppositely signed, as predicted by CN theory
cns1<-
"
# latent variables
lrs2 =~ rs1t2 + rs2t2 + rs3t2 + rs4t2 + rs5t2 + rs6t2
lrc2 =~ rci1t2 + rci2t2 + rci3t2 + rci4t2
lrp2 =~ rpa1t2 + rpa2t2 + rpa3t2
lrcn2 =~ rct1 + rct2 + rct3 + rct4 + rct5
# main regressions
lrs2 ~ a1*lrp2 + a2*lrcn2 
lrc2 ~ b*lrs2+ c1*lrp2 + c2*lrcn2 
# indirect effects
iis := a1*b
icn := a2*b
# diff b/w indirect effects
diffi := icn - iis
# total effects
tis := c1 + (a1*b)
tcn := c2 + (a2*b)
# diff b/w total effects
difft1 := tcn - tis
# abs diff b/w total effects
difft2 := tcn + tis
"
set.seed(1234)
cns1.fit <- sem(cns1, data = Data, 
              #estimator = 'MLM',                        # use to get Satorra Bentler estimates
              se = "bootstrap", bootstrap = 5000,     # use to get bootstrapped IEs
              h1.information="unstructured")
summary(cns1.fit, standardized = T, fit.measures = T, ci=T)

### constraining effects of CN and IS to equality in SI threat eq. reduces fit,
### so CN more strongly related than IS to SI threat
cns1a<-
  "
# latent variables
lrs2 =~ rs1t2 + rs2t2 + rs3t2 + rs4t2 + rs5t2 + rs6t2
...
lrcn2 =~ rct1 + rct2 + rct3 + rct4 + rct5
# main regressions
lrs2 ~ a1*lrp2 + a1*lrcn2 
lrc2 ~ b*lrs2+ c1*lrp2 + c2*lrcn2 
"
set.seed(1234)
cns1a.fit <- sem(cns1a, data = Data, 
                # estimator = 'MLM',                        # use to get Satorra Bentler estimates
                se = "bootstrap", bootstrap = 5000,     # use to get bootstrapped IEs
                h1.information="unstructured")
summary(cns1a.fit, standardized = T, fit.measures = T, ci=T)

# lavTestLRT(cns1.fit, cns1a.fit, method = "satorra.bentler.2001")
anova(cns1.fit, cns1a.fit)

###### robustness check with covariates in structural model:
### baseline model and indirect effects
### 1. CN has signif indirect effect on conspiracy via SI threat
### 2. indirect effect of CN via SI threat > indirect effect of IS via SI threat
### 3. Total effect of CN on consp. larger in abs terms than total effect of IS
...
lrs2 ~ age + gend + whitet1 + rinct1 + educ + rknt1 + ridt2 + idem + irep
lrc2 ~ age + gend + whitet1 + rinct1 + educ + rknt1 + ridt2 + idem + irep
# indirect effects
iis := a1*b
icn := a2*b
# diff b/w indirect effects
diffi1 := icn - iis
# abs diff between indirect effects
diffi2 := icn - iis
# total effects
tis := c1 + (a1*b)
tcn := c2 + (a2*b)
...
"
set.seed(1234)
cns2.fit <- sem(cns2, data = Data, 
                # estimator = 'MLM',                        # use to get Satorra Bentler estimates
                se = "bootstrap", bootstrap = 5000,     # use to get bootstrapped IEs
                h1.information="unstructured")
summary(cns2.fit, standardized = T, fit.measures = T, ci=T)
...
"
set.seed(1234)
cns2a.fit <- sem(cns2a, data = Data, 
                 # estimator = 'MLM',                        # use to get Satorra Bentler estimates
                 se = "bootstrap", bootstrap = 5000,     # use to get bootstrapped IEs
                 h1.information="unstructured")
summary(cns2a.fit, standardized = T, fit.measures = T, ci=T)

# lavTestLRT(cns2.fit, cns2a.fit, method = "satorra.bentler.2001")
anova(cns2.fit, cns2a.fit)
